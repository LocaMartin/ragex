...
normalize_targets() {
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        # Extract any real URLs embedded inside descriptive scope lines.
        # Example: "any applications under https://www.expressvpn.com/latest"
        printf '%s\n' "$line" \
            | grep -Eoi "https?://[^[:space:]<>\"')\\]}]+" 2>/dev/null \
            | sed -E 's#^(https?://[^/?#]+).*#\1#' \
...
export CONNECT_TIMEOUT MAX_TIME RETRIES MAX_SITEMAPS UA RESULTS_DIR
